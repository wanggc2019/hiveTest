...
public class MergeHdfsFiles {
    ...
    private static Logger logger = LoggerFactory.getLogger(MergeHdfsFiles.class);
    ...
    private Set<String> needMergerFolderPath = new HashSet<String>();
...
//        }
        MergeHdfsFiles mergeSmallFile = new MergeHdfsFiles();
        //GenericOptionsParser 命令行解析器,getRemainingArgs()方法则得到剩余的参数
        String[] uargs = new GenericOptionsParser(configuration, args).getRemainingArgs();
        defaultFS = "hdfs://134.64.14.231:8020";
        folderPath = "/user/lff";
        if (uargs.length == 3) {
